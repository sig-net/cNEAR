...
**Ambiguous submissions:** A timeout after broadcast does not prove failure. The deployer broadcasts exactly once via `broadcast_tx_commit`; it retains the exact signed transaction hash, then uses typed JSON-RPC status queries (`tx`) until success/failure or a bounded unresolved timeout. It refuses to submit a duplicate transaction while status is unknown. Save the reported hash when an unresolved error occurs and inspect it before taking any manual recovery action.
