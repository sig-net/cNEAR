...
fn print_dry_run(config: &Config) {
    ...
    println!("{GREEN}=== Step 3: Transfer Token Ownership to Controller ==={NC}");
    print_command(&format!(
    ...
    println!("{GREEN}=== Step 4: Verify Ownership ==={NC}");
    print_command(&format!(
...
async fn deploy_contracts(
    ...
    success();
    println!("{GREEN}=== Step 3: Transfer Token Ownership to Controller ==={NC}");
    // owner_set is not #[payable] in the contract, so no deposit may be attached.
    signer
        ...
        .await?
        .into_result()
        .context("token ownership transfer failed")?;
    success();
    println!("{GREEN}=== Step 4: Verify Ownership ==={NC}");
